...
代码链接：
##### API使用比较分析
• 语音识别API一开始我使用的是百度API，最后我使用的也是百度API，从识别的准确性来看，科大讯飞的语音识别在市场上的认可度比较高，可以达到97%，另外，科大讯飞优势在于大段大段的文字识别上，准确率较高，比较符合我要输入整个行程的想法，但是它没有python的SDK档，所以我做不出，退而求其次使用百度语音识别API,
##### API使用风险报告
